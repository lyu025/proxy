...
export class M3U8Rewriter{
	/**
	 * 初始化重写器
	 * @param {string} proxyBaseUrl - 代理服务器基础URL
	 * @param {Object} config - 重写配置选项
	 */
	constructor(proxyBaseUrl,config={}){
	...
	createProxiedUrl(originalUrl){
		try{
			...
			return`${this.proxyBaseUrl}/o?u=${encodeURIComponent(target)}`;
		}catch{
			return`${this.proxyBaseUrl}/o?u=${encodeURIComponent(originalUrl)}`;
		}
	}
